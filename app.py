...
def render_watchlist_signal_monitor(mapping_text: str):
    ...
    if st.button("Signale aktualisieren"):
        ...
        with st.spinner("Berechne V2-Fruehsignale..."):
            ...
            for i, entry in enumerate(entries):
                progress_bar.progress((i) / len(entries), text=f"Lade Signale fÃ¼r {entry} ({i}/{len(entries)})...")
    ...
    if delta_items:
        ...
        if hard_alerts:
            st.warning(f"ðŸš¨ {len(hard_alerts)} Aktien ueberschreiten den Alert-Schwellwert von +{alert_threshold} Punkten seit dem letzten Snapshot!")
    ...
    st.markdown("---")
    st.subheader("ðŸ” Detail-Analyse")
